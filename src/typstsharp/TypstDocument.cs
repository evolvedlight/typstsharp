...
public sealed class TypstDocument : IDisposable
{
    ...
    internal unsafe TypstDocument(CsBindgen.CompileResult native)
    {
        ...
        var warnings = new string[checked((int)native.warnings_len)];
        for (int i = 0; i < warnings.Length; i++)
        {
            warnings[i] = Marshal.PtrToStringUTF8((nint)native.warnings[i].message) ?? string.Empty;
        }
        ...
        // The rendered bytes are invisible to the GC now that they no longer live on the managed
        // heap. Without this a leaked document exerts no collection pressure at all.
    ...
    private static SafeFileHandle OpenFile(string path, long length, FileOptions options) =>
        File.OpenHandle(
            ...
            FileMode.Create,
            FileAccess.Write,
            // File.WriteAllBytes, which this replaces, permits concurrent readers.
            FileShare.Read,
            options,
    ...
    private sealed unsafe class OutputStream : UnmanagedMemoryStream
    {
        ...
        // covers them all. Overriding Read(Span) as well would recurse: UnmanagedMemoryStream does
        // not override it, and the Stream implementation it inherits calls back into Read(byte[]).
